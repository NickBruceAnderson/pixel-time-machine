...
const SMASH_WINDOW_MS     = 300;    // how long after pressing the smash key a smash can fire (milliseconds)
const SMASH_SPEED_MULT    = 2;      // speed multiplier when a smash connects
// ─────────────────────────────────────────────────────────────────────────────
...
class PongScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Timestamps of the last smash key press for each player (-Infinity = not pressed)
    ...
    update(time, delta) {
        ...
        // ── Smash inputs ─────────────────────────────────────────────────────
        // Record the timestamp so checkPaddleCollision can detect a timed smash
    ...
    checkPaddleCollision(paddle, deflectDir, smashAt, currentTime) {
        ...
        // Smash: if the player pressed their smash key within SMASH_WINDOW_MS, double the speed
        if (currentTime - smashAt <= SMASH_WINDOW_MS) {
            this.ballSpeed *= SMASH_SPEED_MULT;
            // Consume the smash so one key press = one smash
            if (deflectDir === 1)  this.smashLeftAt  = -Infinity;
            if (deflectDir === -1) this.smashRightAt = -Infinity;
        }
